...
int main()
{
  ...
  h.onMessage([&ukf,&target_x,&target_y](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
	  ...
	  target_x  = ukf.x_[0];
	  target_y  = ukf.x_[1];
    // construct target velocity from estimates
    double target_vx = ukf.x_[2]*cos(ukf.x_[3]);
    double target_vy = ukf.x_[2]*sin(ukf.x_[3]);
    // we are given that max speed for both vehicles is the same and, by observation,
    // target vehicle estimated velocity is nearly constant, so use estimated target velocity
    // as the hunter vehicle velocity
    double hunter_v  = ukf.x_[2];
    // compute relative positions
    double  x = target_x  - hunter_x;
    double  y = target_y  - hunter_y;
    // compute relative velocities
    // NOTE: heading definition is relative to due north (y in track frame), so the 
    // x,y velocities of hunter vehicle are obtained by rotating track frame by 90deg
    // (this results in swapping of -sin for cos and cos for sin in the usual vx,vy
    // calculations)
    double vx = target_vx + hunter_v*sin(hunter_heading*M_PI/180.);
    double vy = target_vy - hunter_v*cos(hunter_heading*M_PI/180.);
    // compute time to point of closest approach (neglects nonlinear effects)
    double rdotv = x*vx + y*vy;
    double vdotv  = vx * vx + vy * vy;
    double tgo = -rdotv/vdotv;
    // compute the zero effort miss by propagating states forward
    double x_zem = x + vx * tgo;
    double y_zem = y + vy * tgo;
    double zem = x_zem * x_zem + y_zem * y_zem;

        double heading_to_target = atan2(y_zem, x_zem);
        while (heading_to_target > M_PI) heading_to_target-=2.*M_PI; 
    	  while (heading_to_target <-M_PI) heading_to_target+=2.*M_PI;
    	  //turn towards the target (based on propagated relative heading)
